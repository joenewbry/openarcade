...
function usage() {
  console.error('Usage: node scripts/generate-unified-design-book.mjs <inputMd> <outputHtml> <title> <subtitle> <playHref> <manifestHref>');
}
...
function renderPage({ title, subtitle, toc, content, sourceRel, playHref, manifestHref }) {
...
<div class="toolbar">
  ...
  <a href="${playHref}">Play Game</a>
  <a href="./${manifestHref}">Art Manifest</a>
</div>
...
const [inputRel, outputRel, title, subtitle, playHref, manifestHref] = args;
const inputAbs = resolve(ROOT, inputRel);
const outputAbs = resolve(ROOT, outputRel);

if (!existsSync(inputAbs)) {
  throw new Error(`Missing input markdown: ${inputRel}`);
}
...
const html = renderPage({ title, subtitle, toc, content, sourceRel, playHref, manifestHref });
